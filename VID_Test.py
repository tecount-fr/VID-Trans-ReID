...
def test(model, queryloader, galleryloader, pool='avg', use_gpu=True,
    ...
    with torch.no_grad():
        for batch_idx, (imgs, pids, camids, img_paths) in enumerate(
                queryloader):
            print(batch_idx)
            if use_gpu:
                Q_imgs = imgs.cuda()
        ...
        data = {
            'qf': qf,  # Query features
            'q_pids': q_pids,  # Query person IDs
            'q_camids': q_camids,  # Query camera IDs
            'q_img_paths': q_img_paths,  # Query image paths
            'gf': gf,  # Gallery features
            'g_pids': g_pids,  # Gallery person IDs
            'g_camids': g_camids,  # Gallery camera IDs
            'g_img_paths': g_img_paths,  # Gallery image paths
        }

        # Specify the path where you want to save the pickle file
        pickle_file_path = 'feature_data.pkl'

        # Save the data to a pickle file
        with open(pickle_file_path, 'wb') as file:
            pickle.dump(data, file)

        print(f"Features and related information saved to {pickle_file_path}")

    print("Computing distance matrix")
    m, n = qf.size(0), gf.size(0)
    distmat = torch.pow(qf, 2).sum(dim=1, keepdim=True).expand(m,
                                                               n) + torch.pow(
        gf, 2).sum(dim=1, keepdim=True).expand(n, m).t()
    ...
    visualize_ranked_results(distmat, (queryloader, galleryloader), "video",
                             width=128, height=256, save_dir='', topk=10)
